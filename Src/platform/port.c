...
//	const uint32_t FlashAddress = 0x080E0000;	//See processor reference manual
...
//	static uint8_t d = 0;
	...
	{														//where the CCS811 VOC sensor loads the BaseLine
	...
	#endif
#if (USE_BKUP_SRAM)
		counter_time = RTC_GetCounter(hrtc);
		/**
		 ...
		 */
		BakUpSRam_Data[0] = BLE_MAGIC_NUMBER;
		BakUpSRam_Data[1] = (uint8_t)(counter_time & 0xFF);
		BakUpSRam_Data[2] = (uint8_t)((counter_time >> 8) & 0xFF);
		BakUpSRam_Data[3] = (uint8_t)((counter_time >> 16) & 0xFF);
		BakUpSRam_Data[4] = (uint8_t)((counter_time >> 24) & 0xFF);
#endif
	}
#if (USE_BKUP_SRAM)
	enable_backup_sram();					//Store the timestamp every second
	writeBkpSram((uint8_t *)BakUpSRam_Data, 6, 0);
	disable_backup_sram();
#endif
...
//	HAL_GPIO_WritePin(IO_CONN1_GPIO_Port, IO_CONN1_Pin, GPIO_PIN_SET);
	...
	if (usart3RxLength)
	{
		...
		huart3.hdmarx->Instance->NDTR = RXBUFFERSIZE;	//Reset DMA pointer
...
void process_timer3_irq(void)
{
	...
	update_5s = true;
	__HAL_TIM_SetCounter(&htim3, 0);	//Clear Timer3
	HAL_NVIC_ClearPendingIRQ(TIM3_IRQn);
...
//		ENS160_SetEnvironmentalData(65.0, 25.0);
	...
	if (!(MidNightPastFive) && (MinMaxStored))
	{
		MinMaxStored = false;
	}
#endif
