...
void USART_Config(UART_HandleTypeDef* uartHandle)
{
	...
	/* Any data received will be stored "RxBuffer" buffer: RXBUFFERSIZE is the number of
	   bytes to receive to trigger usart callback */

	HAL_UART_MspInit(uartHandle);
	memset(aRxBuffer, 0x00, RXBUFFERSIZE);
    __HAL_UART_ENABLE_IT(uartHandle, UART_IT_RXNE);	//Enable UART read data register not empty interrupt
