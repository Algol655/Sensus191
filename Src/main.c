...
#include "application/MEMS_app.h"
	...
	#elif (CENTRAL_APP)
		#include "Central/app_bluenrg_2.h"
	#elif (SERIAL_PORT_APP)
		#include "TraSer/app_bluenrg_2.h"
	#elif (SENSOR_APP)
	...
	#include "LoRa_OpModes.h"
		...
		#include "SX127X/app_sx127x_lora.h"
	#elif (CONCENTRATOR_APP)
		#include "Central/app_sx127x_lora.h"
	#elif (LoRa_SERIAL_PORT_APP)
		#include "TraSer/app_sx127x_lora.h"
	#endif
#endif
#if (IO_EXP_PRESENT==1)
...
//HAL_GPIO_WritePin(PC_11_DISC_GPIO_Port, PC_11_DISC_Pin, GPIO_PIN_RESET);	//Pull-up Enable on USBDP (Only for Olimex STM32-H103 Board)
  ...
  WarmUpPeriod_expired = false; ColdRestart = false; BakUpSRamWiped = false; BLE_DataReady = false; LoRa_DataReady = false;
...
//  HAL_NVIC_DisableIRQ(EXTI0_IRQn);			//Made by BSP
  ...
  /* USER CODE BEGIN WHILE */
	while (1)
	{
#if ((GUI_SUPPORT==0) && (BLE_SUPPORT==0) && (LoRa_SUPPORT==0))
...
//  MX_X_CUBE_MEMS1_Process();
		...
		#endif
	#elif ((LoRa_SUPPORT) && (SENSOR_END_NODE_APP))
		if ((LoRa_DataReady) && (ServiceTimer5.Expired))
			MX_SX127X_LoRa_Process();
		#if (USE_IWDGT)
		else
			HAL_IWDG_Refresh(&hiwdg);
	#endif
...
//			send_usart3message(&dataseq[0], Message_Length);//Data to be displayed by STM UnicleoGUI
	...
	#endif	//(GUI_SUPPORT==0) && (BLE_SUPPORT==0) && (LoRa_SUPPORT==0)
	#if	(USB_SUPPORT==1)
		usb_run();
	#endif
	#if (USART_SUPPORT==1)
