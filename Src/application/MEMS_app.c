...
uint8_t HW_Version[5] ="2000";	//Only the first two digits are used!!
...
void Store_MeanValues_BackupRTC(void)
{
	...
	HOST_TO_BKPR_LE_16(BakUpSRam_Data+6, eq_TVOC_1h_Mean);
	HOST_TO_BKPR_LE_16(BakUpSRam_Data+8, eq_CO2_1h_Mean);
	//Store Air Quality Data. Max values
	HOST_TO_BKPR_LE_16(BakUpSRam_Data+48, eq_TVOC_1h_Mean_Max);
	HOST_TO_BKPR_LE_16(BakUpSRam_Data+50, eq_CO2_1h_Mean_Max);
#endif
#if (GAS_SENSOR_MODULE_PRESENT)
	//Store Air Quality Data. Averaged values
	HOST_TO_BKPR_LE_16(BakUpSRam_Data+10, CO_8h_Mean);
	HOST_TO_BKPR_LE_16(BakUpSRam_Data+16, CH2O_8h_Mean);
	HOST_TO_BKPR_LE_16(BakUpSRam_Data+12, NO2_1h_Mean);
	HOST_TO_BKPR_LE_16(BakUpSRam_Data+14, NH3_8h_Mean);
	//Store Air Quality Data. Daily Max values
	HOST_TO_BKPR_LE_16(BakUpSRam_Data+52, CO_8h_Mean_Max);
	HOST_TO_BKPR_LE_16(BakUpSRam_Data+54, CH2O_8h_Mean_Max);
	HOST_TO_BKPR_LE_16(BakUpSRam_Data+56, NO2_1h_Mean_Max);
	HOST_TO_BKPR_LE_16(BakUpSRam_Data+58, NH3_8h_Mean_Max);
	#if (OUTDOOR_MODE)
		//Store Air Quality Data. Averaged values
		HOST_TO_BKPR_LE_16(BakUpSRam_Data+18, O3_1h_Mean);
		HOST_TO_BKPR_LE_16(BakUpSRam_Data+20, SO2_1h_Mean);
		HOST_TO_BKPR_LE_16(BakUpSRam_Data+22, C6H6_24h_Mean);
		//Store Air Quality Data. Daily Max values
		HOST_TO_BKPR_LE_16(BakUpSRam_Data+60, O3_1h_Mean_Max);
		HOST_TO_BKPR_LE_16(BakUpSRam_Data+62, SO2_1h_Mean_Max);
		HOST_TO_BKPR_LE_16(BakUpSRam_Data+64, C6H6_24h_Mean_Max);
	#endif
#endif	//GAS_SENSOR_MODULE_PRESENT
#if (PARTICULATE_SENSOR_PRESENT)
	//Store Air pollution Data. Averaged values
	HOST_TO_BKPR_LE_16(BakUpSRam_Data+24, MC_1p0_24h_Mean);
	HOST_TO_BKPR_LE_16(BakUpSRam_Data+26, MC_2p5_24h_Mean);
	HOST_TO_BKPR_LE_16(BakUpSRam_Data+28, MC_4p0_24h_Mean);
	HOST_TO_BKPR_LE_16(BakUpSRam_Data+30, MC_10p0_24h_Mean);
	//Store Air pollution Data. Daily Max values
	HOST_TO_BKPR_LE_16(BakUpSRam_Data+66, MC_2p5_24h_Mean_Max);
	HOST_TO_BKPR_LE_16(BakUpSRam_Data+68, MC_10p0_24h_Mean_Max);
#endif	//PARTICULATE_SENSOR_PRESENT
	//Store environmental data. Daily Min-Max values
#if (PRESSURE_SENSOR_PRESENT)
	HOST_TO_BKPR_LE_32(BakUpSRam_Data+32, P_Min);
	HOST_TO_BKPR_LE_32(BakUpSRam_Data+36, P_Max);
#endif
#if (HUMIDITY_SENSOR_PRESENT)
	HOST_TO_BKPR_LE_16(BakUpSRam_Data+44, H_Min);
	HOST_TO_BKPR_LE_16(BakUpSRam_Data+46, H_Max);
#endif
	...
	HOST_TO_BKPR_LE_16(BakUpSRam_Data+90, AH_Min);
	HOST_TO_BKPR_LE_16(BakUpSRam_Data+92, AH_Max);
#endif
#if ((PRESSURE_SENSOR_PRESENT) || (HUMIDITY_SENSOR_PRESENT))
	HOST_TO_BKPR_LE_16(BakUpSRam_Data+40, T_Min);
	HOST_TO_BKPR_LE_16(BakUpSRam_Data+42, T_Max);
#endif
#if ((UVx_SENSOR_PRESENT) && (defined(STM32F405xx)))	//The size of the backup SRAM (BKPSRAM) in the STM32F1xx family is only 84 bytes.
	#if (LTR390UV)
		HOST_TO_BKPR_LE_32(BakUpSRam_Data+78, Lux_Min);
		HOST_TO_BKPR_LE_32(BakUpSRam_Data+82, Lux_Max);
	#endif
	HOST_TO_BKPR_LE_32(BakUpSRam_Data+86, UV_Index_Min);
	HOST_TO_BKPR_LE_32(BakUpSRam_Data+90, UV_Index_Max);
#endif
	//Store Global Status Register
	HOST_TO_BKPR_LE_32(BakUpSRam_Data+70, StatusReg);
	HOST_TO_BKPR_LE_32(BakUpSRam_Data+74, Up_Time_H);
...
void ReStore_MeanValues_BackupRTC(void)
{
	...
	memcpy(&eq_TVOC_1h_Mean, &BakUpSRam_Data[6], 2);
	memcpy(&eq_CO2_1h_Mean, &BakUpSRam_Data[8], 2);
	//Restore Air Quality Data. Daily Max values
	memcpy(&eq_TVOC_1h_Mean_Max, &BakUpSRam_Data[48], 2);
	memcpy(&eq_CO2_1h_Mean_Max, &BakUpSRam_Data[50], 2);
#endif
#if (GAS_SENSOR_MODULE_PRESENT)
	//Restore Air Quality Data. Averaged values
	memcpy(&CO_8h_Mean, &BakUpSRam_Data[10], 2);
	memcpy(&CH2O_8h_Mean, &BakUpSRam_Data[16], 2);
	memcpy(&NO2_1h_Mean, &BakUpSRam_Data[12], 2);
	memcpy(&NH3_8h_Mean, &BakUpSRam_Data[14], 2);
	//Restore Air Quality Data. Max values
	memcpy(&CO_8h_Mean_Max, &BakUpSRam_Data[52], 2);
	memcpy(&CH2O_8h_Mean_Max, &BakUpSRam_Data[54], 2);
	memcpy(&NO2_1h_Mean_Max, &BakUpSRam_Data[56], 2);
	memcpy(&NH3_8h_Mean_Max, &BakUpSRam_Data[58], 2);
	#if (OUTDOOR_MODE)
		//Restore Air Quality Data. Averaged values
		memcpy(&O3_1h_Mean, &BakUpSRam_Data[18], 2);
		memcpy(&SO2_1h_Mean, &BakUpSRam_Data[20], 2);
		memcpy(&C6H6_24h_Mean, &BakUpSRam_Data[22], 2);
		//Restore Air Quality Data. Daily Max values
		memcpy(&O3_1h_Mean_Max, &BakUpSRam_Data[60], 2);
		memcpy(&SO2_1h_Mean_Max, &BakUpSRam_Data[62], 2);
		memcpy(&C6H6_24h_Mean_Max, &BakUpSRam_Data[64], 2);
	#endif
#endif	//GAS_SENSOR_MODULE_PRESENT
#if (PARTICULATE_SENSOR_PRESENT)
	//Restore Air pollution Data. Averaged values
	memcpy(&MC_1p0_24h_Mean, &BakUpSRam_Data[24], 2);
	memcpy(&MC_2p5_24h_Mean, &BakUpSRam_Data[26], 2);
	memcpy(&MC_4p0_24h_Mean, &BakUpSRam_Data[28], 2);
	memcpy(&MC_10p0_24h_Mean, &BakUpSRam_Data[30], 2);
	//Restore Air pollution Data. Daily Max values
	memcpy(&MC_2p5_24h_Mean_Max, &BakUpSRam_Data[66], 2);
	memcpy(&MC_10p0_24h_Mean_Max, &BakUpSRam_Data[68], 2);
#endif	//PARTICULATE_SENSOR_PRESENT
	//Restore environmental Data. Daily Min-Max values
#if (PRESSURE_SENSOR_PRESENT)
	memcpy(&P_Min, &BakUpSRam_Data[32], 4);
	memcpy(&P_Max, &BakUpSRam_Data[36], 4);
#endif
#if (HUMIDITY_SENSOR_PRESENT)
	memcpy(&H_Min, &BakUpSRam_Data[44], 2);
	memcpy(&H_Max, &BakUpSRam_Data[46], 2);
#endif
#if ((HUMIDITY_SENSOR_PRESENT) && (defined(STM32F405xx)))	//The size of the backup SRAM (BKPSRAM) in the STM32F1xx family is only 84 bytes.
	memcpy(&AH_Min, &BakUpSRam_Data[90], 2);
	memcpy(&AH_Max, &BakUpSRam_Data[92], 2);
#endif
#if ((PRESSURE_SENSOR_PRESENT) || (HUMIDITY_SENSOR_PRESENT))
	memcpy(&T_Min, &BakUpSRam_Data[40], 2);
	memcpy(&T_Max, &BakUpSRam_Data[42], 2);
#endif
#if ((UVx_SENSOR_PRESENT) && (defined(STM32F405xx)))	//The size of the backup SRAM (BKPSRAM) in the STM32F1xx family is only 84 bytes.
	#if (LTR390UV)
		memcpy(&Lux_Min, &BakUpSRam_Data[78], 4);
		memcpy(&Lux_Max, &BakUpSRam_Data[82], 4);
	#endif
	memcpy(&UV_Index_Min, &BakUpSRam_Data[86], 4);
	memcpy(&UV_Index_Max, &BakUpSRam_Data[90], 4);
#endif
	//Restore Global Status Register
	memcpy(&StatusReg, &BakUpSRam_Data[70], 4);
	//Restore the UpTime Register
	memcpy(&Up_Time_H, &BakUpSRam_Data[74], 4);
/*
