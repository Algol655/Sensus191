...
{
	ServiceTimer1.TimeOut = ServiceTimer1_Timeout;
	ServiceTimer1.Counter = 0;
	ServiceTimer1.Start = false;
	ServiceTimer1.Expired = false;

	ServiceTimer2.TimeOut = ServiceTimer2_Timeout;
	ServiceTimer2.Counter = 0;
	ServiceTimer2.Start = false;
	ServiceTimer2.Expired = false;

	ServiceTimer3.TimeOut = ServiceTimer3_Timeout;
...
{
	HAL_TIM_Base_Start_IT(htim);			//ReStart Timer
...
void TIM_OC_Timers_Stop(TIM_HandleTypeDef *htim)
{
	...
	__HAL_TIM_SetCounter(htim, 0);		//Clear Timer
	HAL_TIM_OC_Stop_IT(htim,TIM_CHANNEL_1);
	__HAL_TIM_SET_COMPARE(htim, TIM_CHANNEL_1, DEFAULT_uhCCR1_Val);
	HAL_TIM_OC_Stop_IT(htim,TIM_CHANNEL_2);
	__HAL_TIM_SET_COMPARE(htim, TIM_CHANNEL_2, DEFAULT_uhCCR2_Val);
	HAL_TIM_OC_Stop_IT(htim,TIM_CHANNEL_3);
	__HAL_TIM_SET_COMPARE(htim, TIM_CHANNEL_3, DEFAULT_uhCCR3_Val);
	HAL_TIM_OC_Stop_IT(htim,TIM_CHANNEL_4);
	__HAL_TIM_SET_COMPARE(htim, TIM_CHANNEL_4, DEFAULT_uhCCR4_Val);
}
